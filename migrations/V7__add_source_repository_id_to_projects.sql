...
SELECT
  ...
  COALESCE(s.source_repository_id, m.source_repository_id, h.source_repository_id) AS source_repository_id
FROM
  spigot_resource s
  ...
  AND LOWER(s.source_repository_id) IS NOT DISTINCT FROM LOWER(m.source_repository_id)
