...
{
    fn from(CommonProjectEntityBorrowed { id,spigot_id,spigot_slug,spigot_name,spigot_description,spigot_author,spigot_version,spigot_premium,spigot_icon_data,modrinth_id,modrinth_slug,modrinth_name,modrinth_description,modrinth_author,modrinth_version,modrinth_icon_url,hangar_slug,hangar_name,hangar_description,hangar_author,hangar_version,hangar_avatar_url,}: CommonProjectEntityBorrowed<'a>) ->
    Self { Self { id,spigot_id,spigot_slug: spigot_slug.map(|v| v.into()),spigot_name: spigot_name.map(|v| v.into()),spigot_description: spigot_description.map(|v| v.into()),spigot_author: spigot_author.map(|v| v.into()),spigot_version: spigot_version.map(|v| v.into()),spigot_premium,spigot_icon_data: spigot_icon_data.map(|v| v.into()),modrinth_id: modrinth_id.map(|v| v.into()),modrinth_slug: modrinth_slug.map(|v| v.into()),modrinth_name: modrinth_name.map(|v| v.into()),modrinth_description: modrinth_description.map(|v| v.into()),modrinth_author: modrinth_author.map(|v| v.into()),modrinth_version: modrinth_version.map(|v| v.into()),modrinth_icon_url: modrinth_icon_url.map(|v| v.into()),hangar_slug: hangar_slug.map(|v| v.into()),hangar_name: hangar_name.map(|v| v.into()),hangar_description: hangar_description.map(|v| v.into()),hangar_author: hangar_author.map(|v| v.into()),hangar_version: hangar_version.map(|v| v.into()),hangar_avatar_url: hangar_avatar_url.map(|v| v.into()),} }
...
CommonProjectEntity, 1>
{
    CommonProjectEntityQuery
    {
        ...
        |row| { CommonProjectEntityBorrowed { id: row.get(0),spigot_id: row.get(1),spigot_slug: row.get(2),spigot_name: row.get(3),spigot_description: row.get(4),spigot_author: row.get(5),spigot_version: row.get(6),spigot_premium: row.get(7),spigot_icon_data: row.get(8),modrinth_id: row.get(9),modrinth_slug: row.get(10),modrinth_name: row.get(11),modrinth_description: row.get(12),modrinth_author: row.get(13),modrinth_version: row.get(14),modrinth_icon_url: row.get(15),hangar_slug: row.get(16),hangar_name: row.get(17),hangar_description: row.get(18),hangar_author: row.get(19),hangar_version: row.get(20),hangar_avatar_url: row.get(21),} }, mapper: |it| { <CommonProjectEntity>::from(it) },
    }
