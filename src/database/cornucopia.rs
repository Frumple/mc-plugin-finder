...
{ pub id : i32,pub name : String,pub tag : String,pub slug : String,pub release_date : time::OffsetDateTime,pub update_date : time::OffsetDateTime,pub author_id : i32,pub version_id : i32,pub version_name : Option<String>,pub premium : Option<bool>,pub source_code_link : Option<String>,}pub struct SpigotResourceEntityBorrowed < 'a >
{ pub id : i32,pub name : &'a str,pub tag : &'a str,pub slug : &'a str,pub release_date : time::OffsetDateTime,pub update_date : time::OffsetDateTime,pub author_id : i32,pub version_id : i32,pub version_name : Option<&'a str>,pub premium : Option<bool>,pub source_code_link : Option<&'a str>,} impl < 'a > From < SpigotResourceEntityBorrowed <
'a >> for SpigotResourceEntity
{
    fn
    from(SpigotResourceEntityBorrowed { id,name,tag,slug,release_date,update_date,author_id,version_id,version_name,premium,source_code_link,} : SpigotResourceEntityBorrowed < 'a >)
    -> Self { Self { id,name: name.into(),tag: tag.into(),slug: slug.into(),release_date,update_date,author_id,version_id,version_name: version_name.map(|v| v.into()),premium,source_code_link: source_code_link.map(|v| v.into()),} }
...
{ UpsertSpigotResourceStmt(cornucopia_async :: private :: Stmt :: new("INSERT INTO spigot_resource (id, name, tag, slug, release_date, update_date, author_id, version_id, version_name, premium, source_code_link)
  ...
  DO UPDATE SET
    ...
    source_code_link = EXCLUDED.source_code_link")) } pub
struct UpsertSpigotResourceStmt(cornucopia_async :: private :: Stmt) ; impl
UpsertSpigotResourceStmt { pub async fn bind < 'a, C : GenericClient, T1 : cornucopia_async::StringSql,T2 : cornucopia_async::StringSql,T3 : cornucopia_async::StringSql,T4 : cornucopia_async::StringSql,T5 : cornucopia_async::StringSql,>
(& 'a mut self, client : & 'a  C,
...
cornucopia_async :: Params < 'a, UpsertSpigotResourceParams < T1,T2,T3,T4,T5,>, std::pin::Pin<Box<dyn futures::Future<Output = Result <
u64, tokio_postgres :: Error > > + Send + 'a>>, C > for UpsertSpigotResourceStmt
{
    fn
    params(& 'a mut self, client : & 'a  C, params : & 'a
    UpsertSpigotResourceParams < T1,T2,T3,T4,T5,>) -> std::pin::Pin<Box<dyn futures::Future<Output = Result < u64, tokio_postgres ::
...
SpigotResourceEntity, 0 >
{
    ...
    {
        client, params : [], stmt : & mut self.0, extractor :
        | row | { SpigotResourceEntityBorrowed { id : row.get(0),name : row.get(1),tag : row.get(2),slug : row.get(3),release_date : row.get(4),update_date : row.get(5),author_id : row.get(6),version_id : row.get(7),version_name : row.get(8),premium : row.get(9),source_code_link : row.get(10),} }, mapper : | it | { <SpigotResourceEntity>::from(it) },
    }
